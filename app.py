...
df = pd.read_csv("https://raw.githubusercontent.com/DiasJoao002/imersao_dados_python_alura/main/dados-finais.csv")
...
empresas_tamanho_disponiveis = sorted(df['tamanho_empresa'].unique())
empresas_tamanho_selecionados = st.sidebar.multiselect("Tamanho da Empresa", empresas_tamanho_disponiveis, default=empresas_tamanho_disponiveis)
...
df_filtrado = df[
    ...
    (df['tamanho_empresa'].isin(empresas_tamanho_selecionados))
]
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        media_do_pais = df_ds.groupby('residencia_iso3')['salario_em_usd'].mean().reset_index()
